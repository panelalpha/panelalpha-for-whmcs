...
use WHMCS\Module\Server\PanelAlpha\Models\ConfigurableOptionGroup;
    ...
    if (isset($GLOBALS['legacyClient'])) {
        ...
        if ($panelAlphaFirstService->configoption5 === 'on') {
            $secondaryNavbar->addChild('panelalpha_sso_link', array(
                'label' => '<span style="margin-left: 12px; margin-right: 12px; color: #5bc0de;" onMouseOver="this.style.textDecoration=\'underline\'"  onMouseOut="this.style.textDecoration=\'none\'">' . $MGLANG['ca']['general']['panelalpha']['sso_link'] . ' <i class="fas fa-external-link"></i></span>',
                'order' => 1,
...
add_hook('AdminProductConfigFields', 1, function ($params) {
    ...
    if ($product->servertype === 'panelalpha' && $_REQUEST['custom'] === 'generate-configurable-options') {
        ...
        $configurableOptionGroup = ConfigurableOptionGroup::updateOrCreate(
            ['name' => "Configurable Options for {$product->name} product"],
            ['description' => "Auto generated by module panelalpha"]
        );
